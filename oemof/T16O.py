...
solph.objective_weighting = [0.2, 0.3]
...
xx = helpers.calculate_timeincrement(date_time_index)
...
energysystem.add(solph.Transformer(
    ...
    outputs={BBEL_SEC: solph.Flow(nominal_value=733.3, variable_costs=19.89, max = var_section_length)},
    conversion_factors={BBEL_SEC: 1, BBNaturalGas: 1.72}))


energysystem.add(solph.Transformer(
    label="BBHCO_P",
    inputs={BBHardCoal: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(nominal_value=0, variable_costs=11.24, max = var_section_length)},
    conversion_factors={BBEL_SEC: 1, BBHardCoal: 2.17}))

energysystem.add(solph.Transformer(
    label="BBLIG_P",
    inputs={BBLignite: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(nominal_value=4409, variable_costs=4.72, max = var_section_length)},
    conversion_factors={BBEL_SEC: 1, BBLignite: 2.33}))

energysystem.add(solph.Transformer(
    label="BBOIL_P",
    inputs={BBOil: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(nominal_value=333.5, variable_costs=25.17, max = var_section_length)},
    conversion_factors={BBEL_SEC: 1, BBOil: 2.61}))

energysystem.add(solph.Transformer(
    label="BBBIO_P",
    inputs={BBBiomass: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(nominal_value=439.8, variable_costs=30.18, max = var_section_length)},
    conversion_factors={BBEL_SEC: 1, BBBiomass: 2.5}))
#Berlin
energysystem.add(solph.Transformer(
    label="BENGA_P",
    inputs={BENaturalGas: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=1040, variable_costs=19.89, max = var_section_length)},
    conversion_factors={BEEL_SEC: 1, BENaturalGas: 1.72}))

energysystem.add(solph.Transformer(
    label="BEHCO_P",
    inputs={BEHardCoal: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=777, variable_costs=11.24, max = var_section_length)},
    conversion_factors={BEEL_SEC: 1, BEHardCoal: 2.17}))

energysystem.add(solph.Transformer(
    label="BELIG_P",
    inputs={BELignite: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=0, variable_costs=4.72, max = var_section_length)},
    conversion_factors={BEEL_SEC: 1, BELignite: 2.33}))

energysystem.add(solph.Transformer(
    label="BEOIL_P",
    inputs={BEOil: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=327, variable_costs=25.17, max = var_section_length)},
    conversion_factors={BEEL_SEC: 1, BEOil: 2.61}))

energysystem.add(solph.Transformer(
    label="BEBIO_P",
    inputs={BEBiomass: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=45.19, variable_costs=30.18, max = var_section_length)},
    conversion_factors={BEEL_SEC: 1, BEBiomass: 2.5}))
...
energysystem.add(solph.Transformer(
    ...
    outputs={BBEL_SEC: solph.Flow(nominal_value=3000, variable_costs=2, max = var_section_length)},
...
energysystem.add(solph.Transformer(
    label="BEINT",
    inputs={BBEL_SEC: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=3000, variable_costs=1, max = var_section_length)},
...
if debug:
    filename = os.path.join(
        helpers.extend_basic_path('lp_files'), r'C:\Users\Winfried\Oemof\results\lp\T16O.lp')
...
results.to_csv(r"C:\Users\Winfried\Oemof\results\NewCapacity\T16O_dfmodel.csv")
...
f = open(r"C:\Users\Winfried\Oemof\results\meta\meta_T16O.txt","w")
...
pd.concat([
    pd.concat([df_node_BBEL_SEC, df_node_BBEL_FIN,df_node_BEEL_SEC, df_node_BEEL_FIN], axis=1)]).to_csv(r'C:\Users\Winfried\Oemof\results\T16O_EL.csv')
