...
# Read data file
filename = os.path.join(os.path.dirname(__file__), r'C:\Users\Winfried\Oemof\BB_BE\T1.csv')
...
energysystem.add(solph.Source(label='BBWIND_P', outputs={BBEL_SEC: solph.Flow(
    actual_value=data['BBWIND_P'], fixed=True, investment=solph.Investment(ep_costs=epc_BBWIND_P))}))
energysystem.add(solph.Source(label='BBSOLPV_P', outputs={BBEL_SEC: solph.Flow(
    actual_value=data['BBSOLPV_P'], fixed=True, investment=solph.Investment(ep_costs=epc_BBSOLPV_P))}))
energysystem.add(solph.Source(label='BBRORHYD_P', outputs={BBEL_SEC: solph.Flow(
    actual_value=data['BBRORHYD_P'], fixed=True, investment=solph.Investment(ep_costs=epc_BBRORHYD_P))}))
#Berlin
energysystem.add(solph.Source(label='BEWIND_P', outputs={BEEL_SEC: solph.Flow(
    actual_value=data['BEWIND_P'], fixed=True, investment=solph.Investment(ep_costs=epc_BEWIND_P))}))
energysystem.add(solph.Source(label='BESOLPV_P', outputs={BEEL_SEC: solph.Flow(
    actual_value=data['BESOLPV_P'], fixed=True, investment=solph.Investment(ep_costs=epc_BESOLPV_P))}))
...
energysystem.add(solph.Transformer(
    ...
    outputs={BBEL_SEC: solph.Flow(variable_costs=19.89*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BESOLPV_P))},
    conversion_factors={BBEL_SEC: 0.581395349}))

energysystem.add(solph.Transformer(
    label="BBHCO_P",
    inputs={BBHardCoal: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(variable_costs=11.24*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BBHCO_P, maximum=0))},
    conversion_factors={BBEL_SEC: 0.460829493}))

energysystem.add(solph.Transformer(
    label="BBLIG_P",
    inputs={BBLignite: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(variable_costs=4.72*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BBLIG_P))},
    conversion_factors={BBEL_SEC: 0.429184549}))

energysystem.add(solph.Transformer(
    label="BBOIL_P",
    inputs={BBOil: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(variable_costs=25.17*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BBOIL_P))},
    conversion_factors={BBEL_SEC: 0.383141762}))

energysystem.add(solph.Transformer(
    label="BBBIO_P",
    inputs={BBBiomass: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(variable_costs=30.18*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BBBIO_P))},
    conversion_factors={BBEL_SEC: 0.4}))
#Berlin
energysystem.add(solph.Transformer(
    label="BENGA_P",
    inputs={BENaturalGas: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(variable_costs=19.89*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BENGA_P))},
    conversion_factors={BEEL_SEC: 0.581395349}))

energysystem.add(solph.Transformer(
    label="BEHCO_P",
    inputs={BEHardCoal: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(variable_costs=11.24*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BEHCO_P))},
    conversion_factors={BEEL_SEC: 0.460829493}))

energysystem.add(solph.Transformer(
    label="BELIG_P",
    inputs={BELignite: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(variable_costs=4.72*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BELIG_P, maximum=0))},
    conversion_factors={BEEL_SEC: 0.429184549}))

energysystem.add(solph.Transformer(
    label="BEOIL_P",
    inputs={BEOil: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(variable_costs=25.17*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BEOIL_P))},
    conversion_factors={BEEL_SEC: 0.383141762}))

energysystem.add(solph.Transformer(
    label="BEBIO_P",
    inputs={BEBiomass: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(variable_costs=30.18*section_length['timesteps_in_section_1'], investment=solph.Investment(ep_costs=epc_BEBIO_P))},
...
energysystem.add(solph.Transformer(
    ...
    outputs={BBEL_SEC: solph.Flow(nominal_value=3000*8784, variable_costs=2*section_length['timesteps_in_section_1'])},
...
energysystem.add(solph.Transformer(
    label="BEINT",
    inputs={BBEL_SEC: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=3000*8784, variable_costs=1*section_length['timesteps_in_section_1'])},
...
if debug:
    filename = os.path.join(
        helpers.extend_basic_path('lp_files'), r'C:\Users\Winfried\Oemof\results\lp\TI1O.lp')
...
f = open(r"C:\Users\Winfried\Oemof\results\meta\meta_TI1O.txt","w")
...
pd.concat([
    pd.concat([df_node_BBEL_SEC, df_node_BBEL_FIN,df_node_BEEL_SEC, df_node_BEEL_FIN], axis=1)]).to_csv(r'C:\Users\Winfried\Oemof\results\TI1O_EL.csv')
