...
# Read data file
filename = os.path.join(os.path.dirname(__file__), 'T1.csv')
data = pd.read_csv(filename)

file_hours = os.path.join(os.path.dirname(__file__), 'CapacityToActivityUnit_oemof.csv')
hours = pd.read_csv(file_hours)
...
energysystem.add(solph.Transformer(
    ...
    outputs={BBEL_SEC: solph.Flow(nominal_value=733.3, variable_costs=19.89, max = 8784)},
...
energysystem.add(solph.Transformer(
    ...
    outputs={BBEL_SEC: solph.Flow(nominal_value=0, variable_costs=11.24, max = 8784)},
    conversion_factors={BBEL_SEC: 1, BBHardCoal: 2.17}))

energysystem.add(solph.Transformer(
    label="BBLIG_P",
    inputs={BBLignite: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(nominal_value=4409, variable_costs=4.72, max = 8784)},
    conversion_factors={BBEL_SEC: 1, BBLignite: 2.33}))

energysystem.add(solph.Transformer(
    label="BBOIL_P",
    inputs={BBOil: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(nominal_value=333.5, variable_costs=25.17, max = 8784)},
    conversion_factors={BBEL_SEC: 1, BBOil: 2.61}))

energysystem.add(solph.Transformer(
    label="BBBIO_P",
    inputs={BBBiomass: solph.Flow()},
    outputs={BBEL_SEC: solph.Flow(nominal_value=439.8, variable_costs=30.18, max = 8784)},
    conversion_factors={BBEL_SEC: 1, BBBiomass: 2.5}))
#Berlin
energysystem.add(solph.Transformer(
    label="BENGA_P",
    inputs={BENaturalGas: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=1040, variable_costs=19.89, max = 8784)},
    conversion_factors={BEEL_SEC: 1, BENaturalGas: 1.72}))

energysystem.add(solph.Transformer(
    label="BEHCO_P",
    inputs={BEHardCoal: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=777, variable_costs=11.24, max = 8784)},
    conversion_factors={BEEL_SEC: 1, BEHardCoal: 2.17}))

energysystem.add(solph.Transformer(
    label="BELIG_P",
    inputs={BELignite: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=0, variable_costs=4.72, max = 8784)},
    conversion_factors={BEEL_SEC: 1, BELignite: 2.33}))

energysystem.add(solph.Transformer(
    label="BEOIL_P",
    inputs={BEOil: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=327, variable_costs=25.17, max = 8784)},
    conversion_factors={BEEL_SEC: 1, BEOil: 2.61}))

energysystem.add(solph.Transformer(
    label="BEBIO_P",
    inputs={BEBiomass: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=45.19, variable_costs=30.18, max = 8784)},
...
energysystem.add(solph.Transformer(
    ...
    outputs={BBEL_SEC: solph.Flow(nominal_value=3000, variable_costs=2, max = 8784)},
...
energysystem.add(solph.Transformer(
    label="BEINT",
    inputs={BBEL_SEC: solph.Flow()},
    outputs={BEEL_SEC: solph.Flow(nominal_value=3000, variable_costs=1, max = 8784)},
...
if debug:
    filename = os.path.join(
        helpers.extend_basic_path('lp_files'), r'C:\Users\Winfried\Oemof\results\lp\T1O.lp')
...
f = open(r"C:\Users\Winfried\Oemof\results\meta\meta_T1O.txt","w")
...
pd.concat([
    pd.concat([df_node_BBEL_SEC, df_node_BBEL_FIN,df_node_BEEL_SEC, df_node_BEEL_FIN], axis=1)]).to_csv(r'C:\Users\Winfried\Oemof\results\T1O_EL.csv')
